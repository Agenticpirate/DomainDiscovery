...
project: DomainDiscovery
source: https://getdesign.md/vercel/design-md
installed: 2026-08-04
note: >
  Primary light-mode design reference for DomainDiscovery. Wire tokens via
  `html.light` in `src/app/globals.css` (`--ds-*` and semantic CSS vars).
  Keep domain status colors (emerald free / amber premium / rose taken) for
  product clarity; brand chrome stays monochrome ink + hairline + link blue.
  Dark mode remains the product default canvas; light mode follows this file.
