      ...
      <main className="relative pt-28">
        ...
        <section className="px-6 pb-8">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className={`text-lg ${isLight ? 'text-slate-500' : 'text-white/50'} max-w-2xl mx-auto mb-8 ${mounted ? 'animate-fade-in' : 'opacity-0'}`}>
        ...
        <section className="px-6 pb-16">
          <div className="max-w-4xl mx-auto">
            {savedDomains.length > 0 ? (
              <>
                {/* Actions Bar */}
                <div className={`flex flex-wrap items-center justify-between gap-4 mb-6 p-4 ${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-white/[0.02] border-white/10'} border rounded-xl`}>
                ...
                <div className="space-y-3">
                  {savedDomains.map((item, index) => (
                    <div
                      key={index}
                      className={`group p-4 ${isLight ? 'bg-white border-slate-200 hover:border-blue-300 hover:shadow-md' : 'bg-white/[0.02] border-white/10 hover:border-white/20 hover:bg-white/[0.04]'} border rounded-xl transition-all`}
                    >
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className={`font-mono text-lg font-semibold ${isLight ? 'text-slate-900' : 'text-white'} mb-1 truncate`}>
                            {item.domain}
                          </div>
                ...
                {/* Privacy Notice */}
                <div className={`mt-8 p-4 ${isLight ? 'bg-emerald-50 border-emerald-200' : 'bg-emerald-500/5 border-emerald-500/20'} border rounded-xl`}>
              ...
              <div className="text-center py-16">
                <div className={`inline-flex p-6 rounded-full ${isLight ? 'bg-slate-100 border-slate-200' : 'bg-white/5 border-white/10'} border mb-6`}>
                ...
                <h3 className={`text-xl font-semibold mb-2 ${isLight ? 'text-slate-800' : 'text-white'}`}>No Saved Domains Yet</h3>
                <p className={`${isLight ? 'text-slate-500' : 'text-white/50'} mb-8 max-w-md mx-auto`}>
