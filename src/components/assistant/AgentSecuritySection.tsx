...
export function AgentSecuritySection({ isLight }: { isLight: boolean }) {
  ...
  return (
    <section id="security" className={`shine-border relative isolate overflow-hidden scroll-mt-20 rounded-2xl border p-4 sm:p-5 ${card}`}>
      ...
      <ul className={`space-y-2 text-xs sm:text-sm ${muted}`}>
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>Auth:</strong> Optional{' '}
        ...
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>Budget hard filter:</strong> Agents should
        ...
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>No unattended spend:</strong>{' '}
        ...
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>Snapshots:</strong> Availability and prices
          change; re-check at registrar checkout.
        </li>
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>Idempotent tools:</strong> Safe to retry tool
          calls; use job ids for long runs.
        </li>
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>Secrets:</strong> Never put registrar API keys
          in browser bundles. Rate-limit agent callers.
        </li>
        <li>
          <strong className={isLight ? 'text-slate-800' : 'text-white/80'}>Industry direction:</strong> GoDaddy Agent Name
