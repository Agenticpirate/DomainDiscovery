  ...
  return (
    <div className="space-y-2 animate-fade-in">
      {results.map((result, i) => (
        <div
          key={i}
          className="glass-card p-4 border-white/10 hover:border-white/20 transition-all duration-300 group animate-fade-in"
        ...
        >
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4 flex-1 min-w-0">
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full ${result.available ? 'bg-emerald-400' : 'bg-white/40'}`} />
            ...
            <div className="flex items-center gap-4 shrink-0">
              {result.available && result.price && (
                <div className="text-right">
                  <span className="text-white/60 font-semibold">{result.price}</span>
                  {result.registrar && (
                    <div className="text-[10px] text-white/30 uppercase tracking-wider">{result.registrar}</div>
                  )}
                </div>
              )}
              {result.seo && (
                <div className="hidden lg:flex items-center gap-3 text-xs text-white/40">
                  {result.seo.traffic && (
